...
namespace
{
    constexpr uint32_t MED_CIPHER = 1344751489;
    constexpr std::array<uint8_t, 256> RIJNDEAL_S_BOX = {
    ...
    constexpr std::array<uint8_t, 256> RIJNDEAL_INVERSE_S_BOX = {
...
uint32_t Crypt::keyGen(uint32_t input)
{
    uint32_t key = randomGen(randomGen(MED_CIPHER ^ input));
...
{
    uint32_t temp = inputData ^ MED_CIPHER;
    for (int i = 0; i < 32; ++i)
    {
...
uint32_t Crypt::simpleDecrypt(uint32_t inputData)
{
    uint32_t temp = inputData;
    for (int i = 0; i < 32; ++i)
    {
        temp = rotatoRight(changeByTable(temp, RIJNDEAL_INVERSE_S_BOX), 32, 1);
    }
    return temp ^ MED_CIPHER;
}

uint32_t Crypt::randomGen(uint32_t input)
{
    const uint32_t a = 16807;
    const uint32_t q = 127773;
    const uint32_t r = 2836;
    int32_t tmp1 = input / q;
    int32_t ret = (input - (tmp1 * q)) * a - (tmp1 * r);
    if (ret < 0)
    {
        ret += 0x7FFFFFFF;
    }
...
uint32_t Crypt::changeByTable(uint32_t inputData, const std::array<uint8_t, 256> &tableData)
{
    std::array<uint8_t, 4> results;

    for (size_t i = 0; i < 4; ++i)
    {
        uint8_t byte = static_cast<uint8_t>(inputData & 0xFF);
        results[i] = tableData[byte];
        inputData >>= 8;
    }
